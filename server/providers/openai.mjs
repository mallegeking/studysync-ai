...
const buildMessages = ({ text, files, customInstructions, previousContent, includeSchemaInPrompt }) => {
...
export async function generate({ text, files, customInstructions, previousContent, apiKey, model, baseUrl, isLocal }) {
  ...
  let response = await callChatCompletions({
    ...
    apiKey,
    model,
    messages: buildMessages({ text, files, customInstructions, previousContent, includeSchemaInPrompt: false }),
    responseFormat: {
      type: 'json_schema',
      json_schema: { name: 'study_material', strict: true, schema: responseJsonSchema },
    },
  });
  ...
  if (!response.ok && isLocal && response.status === 400) {
    response = await callChatCompletions({
      ...
      apiKey,
      model,
      messages: buildMessages({ text, files, customInstructions, previousContent, includeSchemaInPrompt: true }),
